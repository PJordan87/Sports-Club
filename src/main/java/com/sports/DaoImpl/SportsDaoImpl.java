...
import com.sports.sportsUtils.Jdbc.DbConnection;
...
public class SportsDaoImpl implements SportsDao{
	...
	public SportsDaoImpl() {
    	System.out.println("SportsDaoImpl");
    }
	...
	public void updateUserById(UserLogin userLogin, long id) {
		...
		String UpdateSql = "UPDATE users SET address=?, age=?, card=?, city=?, emailAddress=?, fname=?,"
		         + " lname=?, state=?, tel=?, zip=?, background=?, selectClub=? where id=? ";    	
	     JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
	  	  jdbcTemplate.update(
	  			UpdateSql,
	  			new Object[] { 
...
*/
	...
	public void savebio(long id) {
		UserLogin userLogin = new UserLogin();
		//userLogin.setId(id);
		UserLogin entity = hibernateUtil.fetchById(id, UserLogin.class);
		entity.setBackground(userLogin.getBackground());
